...
def get_history():
    response = requests.get('http://your-api-url/api/history')
...
def process_image_overlay(files, data):
    url = 'http://your-api-url/api/image-overlay'
    headers = {'Content-Type': 'multipart/form-data'}
    files_data = [('files', file) for file in files]
    form_data = {'data': json.dumps(data)}
    response = requests.post(url, files=files_data, data=form_data, headers=headers)
    if response.status_code == 200:
