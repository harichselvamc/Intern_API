...
#         history_data = load_json_data()
#         json_data = json.dumps(history_data, indent=4)
#         b64 = base64.b64encode(json_data.encode()).decode()
#         href = f'<a href="data:application/json;base64,{b64}" download="history.json">Download history.json File</a>'
#         st.markdown(href, unsafe_allow_html=True)

#     if st.button('Download Overlayed Images'):
#         format_options = ['PNG', 'JPEG', 'SVG']
#         image_format = st.selectbox('Select image format', format_options)
...
#             image_with_overlay.save(image_bytes, format=image_format)
#             image_bytes.seek(0)
#             b64 = base64.b64encode(image_bytes.read()).decode()
#             href = f'<a href="data:image/{file_extension};base64,{b64}" download="image{i+1}.{file_extension}">Download Image {i+1} ({image_format})</a>'
#             st.markdown(href, unsafe_allow_html=True)
...
def main():
    ...
    if uploaded_files:
        ...
        if st.button('Save All'):
            for data in image_data_list:
                save_to_history(data)
            st.success('All data saved to history.json')
    ...
    if st.button('Download History JSON'):
        ...
        json_data = json.dumps(history_data, indent=4)
        b64 = base64.b64encode(json_data.encode()).decode()
        href = f'<a href="data:text/json;base64,{b64}" download="history.json">Download history.json File</a>'
        st.markdown(href, unsafe_allow_html=True)
