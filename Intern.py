...
def overlay_image():
    ...
    for image in images_with_overlay:
        buffered = io.BytesIO()
        image.save(buffered, format="PNG")
        encoded_image = base64.b64encode(buffered.getvalue()).decode()
        base64_images.append(encoded_image)

    return jsonify({"images_with_overlay": base64_images})
...
def save_to_history_api():
    data = request.json
    save_to_history(data)
    return jsonify({"message": "Data saved to history.json"})


@app.route('/api/history/html', methods=['GET'])
def download_history_html():
    ...
    with open('history.html', 'r') as file:
        b64 = base64.b64encode(file.read().encode()).decode()
    return jsonify({"history_html_base64": b64})
...
def main():
    ...
    if st.button('Download History HTML'):
        response = requests.get('http://localhost:5000/api/history/html')
        if response.status_code == 200:
            base64_html = response.json()['history_html_base64']
            href = f'<a href="data:text/html;base64,{base64_html}" download="history.html">Download history.html File</a>'
            st.markdown(href, unsafe_allow_html=True)
