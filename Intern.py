...
def add_image_overlay(images, image_data_list):
    images_with_overlay = []
    for image, data in zip(images, image_data_list):
        # Add overlay logic here and append the resulting image to images_with_overlay list
        images_with_overlay.append(image)
    return images_with_overlay
...
def generate_html(data):
    ...
    for entry in data:
        ...
        altered_size = entry.get('altered_size', '')
...
def main():
    ...
    if uploaded_files:
        ...
        for i, uploaded_file in enumerate(uploaded_files):
            ...
            font_size = st.number_input(f'Overlay font size for Image {i+1}', min_value=1, value=20)
            position = st.selectbox(f'Overlay position for Image {i+1}', ('bottom-left', 'bottom-right', 'top-left', 'top-right'))
            font_color = st.color_picker(f'Overlay font color for Image {i+1}', value='#FFFFFF')
            altered_size = st.checkbox(f'Altered size for Image {i+1}')
