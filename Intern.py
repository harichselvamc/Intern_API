...
@app.post("/overlay")
def overlay_images(image_data_list: list, images: list):
    images_with_overlay = add_image_overlay(images, image_data_list)
    return {"images_with_overlay": images_with_overlay}


@app.post("/resize")
def resize_images(image_with_overlay_list: list, sizes: list):
    resized_images = []

    for image_with_overlay, size in zip(image_with_overlay_list, sizes):
        if size == "Default":
            resized_image_with_overlay = image_with_overlay
            altered_size = "Default"
        else:
        ...
        resized_images.append(resized_image_with_overlay)

    return {"resized_images": resized_images}


@app.post("/save")
def save_to_history_endpoint(data_list: list):
...
def main():
    st.title('Image Overlay API')
    st.write("To interact with the API, please use the provided endpoints.")

    if st.button('View History'):
        history_data = view_history()
        st.write(history_data)

    if st.button('Download History HTML'):
        history_html = download_history_html()
        st.write(history_html)

    if st.button('Download History JSON'):
        history_json = download_history_json()
